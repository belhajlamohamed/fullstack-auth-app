...
export default function Dashboard() {
  ...
  const [userRole] = useState((localStorage.getItem("userRole") || "").toLowerCase());
  ...
  const renderContent = () => {
    ...
    console.log("Rendu - Vue:", activeView, "| Rôle traité:", userRole);
    ...
    return (
      <div className="p-20 flex flex-col items-center justify-center border-2 border-red-500/20 rounded-[3rem] bg-red-500/5">
        ...
        <p className="text-gray-500 text-sm mt-2">Rôle détecté : <span className="text-white font-mono bg-white/10 px-2 rounded">{userRole}</span></p>
      </div>
    );
  };
